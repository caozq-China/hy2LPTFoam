...
:
    initialiseDict_(solidInitialiseDict),
    configurationList_(initialiseDict_.lookup("configurations")),
    ...
    configurations_(configurationList_.size())
{
    ...
    if(configurations_.size() > 0 )
    {
        forAll(configurations_, c)
        {
            ...
            configurations_[c] = configuration::New(cloud, configurationIDict);
...
void allConfigurations::setInitialConfig()
{
    forAll(configurations_, c)
    {
