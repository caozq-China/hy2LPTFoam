...
defineTypeNameAndDebug(Explicit, 0);

addToRunTimeSelectionTable(PackingModel, Explicit, dictionary);

}

// * * * * * * * * * * * * * * * * Constructors  * * * * * * * * * * * * * * //

Foam::Explicit::Explicit
(
...
:
    ...
    (
        ...
        (
            dict.subDict(CorrectionLimitingMethod::typeName)
        )
    )
{}
...
// template<class CloudType>
void Foam::Explicit::cacheFields(const bool store)
{
...
// template<class CloudType>
Foam::vector Foam::Explicit::velocityCorrection
(
...
bool Foam::Explicit::active() const
{
    return true;
}
