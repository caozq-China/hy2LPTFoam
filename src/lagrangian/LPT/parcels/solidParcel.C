...
void Foam::solidParcel::calcUCorrection
(
    trackingData& td,
    const scalar dt
)
{
    ...
    p.UCorrect() = Zero;
    if(td.cloud().interiorInteractionType()==2)
    {
        if(td.cloud().dampingModel().active())
        {
            p.UCorrect() = td.cloud().dampingModel().velocityCorrection(p, dt);
        }
        if(td.cloud().packingModel().active())
        {
            p.UCorrect() += td.cloud().packingModel().velocityCorrection(p, dt);
        }
    }
}
...
)
{
    ...
    while (td.keepParticle && !td.switchProcessor && tEnd > ROOTVSMALL)
    {
        ...
        if ((dt > ROOTVSMALL))
        {
            ...
            p.calcUCorrection(td,dt);//- MPPIC update UCorrect
