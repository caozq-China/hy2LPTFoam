...
Foam::dsmcReactions::dsmcReactions(const Time& t, const polyMesh& mesh)
:
    time_(t),
    chemReactDict_
    (
...
Foam::dsmcReactions::dsmcReactions
(
    const Time& t,
    const polyMesh& mesh,
    dsmcCloud& cloud
    ...
    counter_(0)
{
    ...
    if (reactions_.size() > 0)
    {
        forAll(reactions_, r)
        {
            ...
            reactions_[r] = dsmcReaction::New(time_, cloud, reactionDict);
...
void Foam::dsmcReactions::initialConfiguration()
{
...
void Foam::dsmcReactions::outputData()
{
    ++counter_;
