...
Foam::dsmcVolFields::dsmcVolFields
(
    const Time& t,
    const polyMesh& mesh,
    dsmcCloud& cloud,
...
void Foam::dsmcVolFields::readIn()
{
    IOdictionary dict
    (
        IOobject
...
void Foam::dsmcVolFields::writeOut()
{
...
void Foam::dsmcVolFields::calculateWallUnitVectors()
{
    forAll(n_, patchi)
    {
...
void Foam::dsmcVolFields::createField()
{
...
void Foam::dsmcVolFields::calculateField()
{
    ++sampleCounter_;
        ...
        if (densityOnly_)
        {
            forAllConstIters(cloud_, iter)
            {
                ...
                if (typeIds_.find(p.typeId()) != -1)
                {
                    ...
                    scalar RWF =
                        cloud_.axiRWF(cloud_.mesh().cellCentres()[cell]);
        ...
        else
        {
            forAllConstIters(cloud_, iter)
            {
                ...
                if (iD != -1)
                {
                    ...
                    linearKEMeanXnParticle_[cell] += mass*(U & U)*RWF*nParticle;
    ...
    if (mesh_.time().writeTime())
    {
        ...
        if (densityOnly_)
        {
            forAll(rhoNMean_, cell)
            {
                ...
                cloud_.fields().dsmcWeight()[cell] = dsmcRhoN_[cell];
            }
        }
        else
        {
            ...
            forAll(rhoNMean_, cell)
            {
                ...
                if (measureErrors_)
                {
                        ...
                        && Cv_p[cell] > VSMALL)
                    {
                        ...
                        pressureError_[cell] =
                            sqrt(gamma)/sqrt(dsmcRhoNMean_[cell]*nTimeSteps_);
                    }
                }
            }
...
void Foam::dsmcVolFields::writeField()
{}

void Foam::dsmcVolFields::updateProperties(const dictionary& dict)
{
