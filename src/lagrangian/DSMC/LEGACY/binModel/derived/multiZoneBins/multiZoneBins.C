    ...
    cellRegionAddressing_()
{
    ...
    // check if a cell zone is defined more than once in the input list
    {
        ...
        forAll(zoneNames, i)
        {
            ...
            if (regionNames.find(zoneName) == -1)
            {
    ...
    forAll(regionIds_, r)
    {
        ...
        forAll(cells, c)
        {
            ...
            {
                FatalErrorInFunction
                    << "There is an overlap present in cell - zone: "
                    << zoneNames[r]
                    << " with zone: "
                    << zoneNames[cellRegionAddressing_[cellI]]
                    << nl << "in: "
                    << mesh_.time().system()/"fieldPropertiesDict"
                    << exit(FatalError);
            }
...
scalarField multiZoneBins::binPositions()
{
    ...
    if (nBins_ > 0)
    {
        ...
        {
            positions[i] = positions[i - 1] + 0.5*binWidths_[i - 1]
                                                + 0.5*binWidths_[i];
        }
    }
...
label multiZoneBins::nBins() const
{
    return nBins_;
}
