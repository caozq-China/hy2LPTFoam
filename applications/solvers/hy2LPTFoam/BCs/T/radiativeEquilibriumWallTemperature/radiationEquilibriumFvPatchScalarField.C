...
#include "radiationEquilibriumFvPatchScalarField.H"
...
:
    ...
    gammaName_("gammatr"),
    mfpName_("mfp"),
    convectiveWHFName_("wallHeatFlux"),
...
radiationEquilibriumFvPatchScalarField
(
    const radiationEquilibriumFvPatchScalarField& ptf,
    const fvPatch& p,
    const DimensionedField<scalar, volMesh>& iF,
...
:
    ...
    convectiveWHFName_(ptf.convectiveWHFName_),
...
:
    ...
    convectiveWHFName_(dict.lookup("wallHeatFluxName")),
...
radiationEquilibriumFvPatchScalarField
(
    const radiationEquilibriumFvPatchScalarField& ptpsf,
    const DimensionedField<scalar, volMesh>& iF
)
:
    mixedFvPatchScalarField(ptpsf, iF),
    accommodationCoeff_(ptpsf.accommodationCoeff_),
    emissivity_(ptpsf.emissivity_),
    Twall_(ptpsf.Twall_)
{}
...
// Map from self
void Foam::radiationEquilibriumFvPatchScalarField::autoMap
(
...
void Foam::radiationEquilibriumFvPatchScalarField::rmap
(
...
void Foam::radiationEquilibriumFvPatchScalarField::updateCoeffs()
{
    if (updated())
    {
        return;
    ...
    const fvPatchScalarField& pwhf =
        patch().lookupPatchField<volScalarField, scalar>(convectiveWHFName_);
...
//    scalarField KEbyRho(0.5*magSqr(pU));
    ...
    forAll(Iw, faceI)
    {
        ...
        // Info<<"pwhf[faceI]="<<pwhf[faceI]<<endl;
        if(pwhf[faceI]>=0)
        {
            Twall_[faceI] = pow(pwhf[faceI]/(emissivity_*5.6704e-08),0.25);
        }
        refValue()[faceI] = Twall_[faceI];
        refGrad()[faceI] = 0.0;
...
// Write
void Foam::radiationEquilibriumFvPatchScalarField::write(Ostream& os) const
{
